...
def addNewLocations(urlNames, keyDF, newRatio=0.5,language='fr', limitCount=9):
...
def getLocationsFromQuote(quote):
    locations = []
        ...
        for entity in doc.ents:
          ...
          if(entity.label_ in ['LOC','GPE']):
           ...
           if(strangeCharacters(locationText,".,!?;:'…<>/\n\r")==0):
             if(not locationText in locations): 
...
def incrementLocationsInKeywords(data):
    ...
    #lang = data['language'] 
    for locationText in getLocationsFromQuote(quote):
...
def checkArticlesForKeywords(articles, keywordsDF, seldomDF, language, keyWord, inclContent=False):
    ...
    for article in articles:
      ...
      foundKeywords = []
      found = False
      # first check if a 'real' location string occurs
      locations = getLocationsFromQuote(searchQuote) 
      for index2, column2 in keywordsLangDF.iterrows(): 
         keyword = column2['keyword']
         for location in locations:
           if(location.lower() == keyword.lower().strip("'")):
...
keywordsDF = addNewLocations('https://raw.githubusercontent.com/mizilett-news/emplacementChuchoter/main/csv/sentiments_new_locations.csv', keywordsDF, 0.5, 'fr', 9)
